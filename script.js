...
function calculateHeartRateZones() {
    ...
    workouts.forEach(workout => {
        ...
        for (const zone in expectedDistribution) {
            ...
            const expectedMidHr = (expectedLowHr + expectedHighHr) / 2;

            if (workout.avgHr >= expectedLowHr && workout.avgHr <= expectedHighHr) {
                // Calculate adjustment as the average of expected and actual HR
                const adjustment = (workout.avgHr - expectedMidHr) * expectedDistribution[zone]; // Corrected line
                zoneAdjustments[zone] += adjustment;
            }
        }
    });
